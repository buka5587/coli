...
if ($cl["config"]["user_wallet_status"] == "off") {
    $data['err_code'] = 0;
    $data['status'] = 400;
}

else if ($action == 'upload_ad_cover' && not_empty($cl['is_logged'])) {
	$data['err_code'] = 0;
    $data['status']   = 400;
    ...
    if (not_empty($ad_data) && $ad_data['user_id'] == $me['id']) {
    	$ad_data_changes  = array(
            ...
            'max_budget'  => fetch_or_get($_POST['max_budget'], false),
    	...
    	foreach ($ad_data_changes as $field_name => $field_val) {
            ...
            else if ($field_name == 'max_budget') {
                if (empty($field_val) || is_posnum($field_val) != true) {
                    $data['err_code'] = "invalid_max_budget"; break;
                }
            }
    	...
    	if (empty($data['err_code'])) {
    		...
    		$ad_update_data   = array(
                ...
                'max_budget'  => cl_text_secure($ad_data_changes['max_budget']),
    ...
    if ($cl['config']['advertising_system'] == 'on') {
        ...
        if (not_empty($ad_data)) {
            ...
            if (not_empty($ad_owner)) {
                ...
                $ad_clicks = ($ad_data['clicks'] += 1);
                $ad_budget = ($ad_data['budget'] += $conv_rate);
                $ad_max_budget = ($ad_data['max_budget'] -= $conv_rate);

                $ad_update_data = array(
                    'clicks' => $ad_clicks,
                    'max_budget' => $ad_max_budget,
                    'budget' => $ad_budget
                );

                if ($ad_data['max_budget'] < 1) {
                    $ad_update_data["status"] = "inactive";
                    $ad_update_data["max_budget"] = 0;
                }
